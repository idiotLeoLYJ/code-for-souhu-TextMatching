...
from models.bert_average_mt_distill import BertTextClassificationWithAveragePooling
from models.bert_cls_avg_mt_distill import BertTextClassificationWithClsAveragePooling
...
MODEL_CLASSES = {
    'roberta-wwm-ext-large': {
        ...
        'cls_avg': BertTextClassificationWithClsAveragePooling
    },
    'mac-bert': {
        'cls': BertTextClassification,
        'average': BertTextClassificationWithAveragePooling,
...
CACHE_NAME = {
    'bert': './cached_data/cached_bert_tokenizer_data_distill',
    'electra': './cached_data/cached_electra_tokenizer_data_distill',
    'roberta-wwm-ext-large': './cached_data/cached_bert_tokenizer_data_distill',
    'mac-bert': './cached_data/cached_bert_tokenizer_data_distill',
}
DATA_DIR = 'logits_data/all_train_data_mean_logits.json'
...
def tokenizer_single(example, args, tokenizer):
    ...
    example_encoding['logits'] = [float(example[4]), float(example[5])]
...
def load_dataset(args, tokenizer):
    ...
    all_logits_train = torch.tensor([f.logits for f in train_features], dtype=torch.float)
    all_types_train = torch.tensor([f.type for f in train_features], dtype=torch.long)
    ...
    all_logits_test = torch.tensor([f.logits for f in test_features], dtype=torch.float)
    all_types_test = torch.tensor([f.type for f in test_features], dtype=torch.long)

    train_dataset = TensorDataset(all_input_ids_train, all_attention_mask_train, all_token_type_ids_train,
                                  all_labels_train, all_types_train, all_logits_train)
    test_dataset = TensorDataset(all_input_ids_test, all_attention_mask_test, all_token_type_ids_test, 
                                 all_labels_test, all_types_test, all_logits_test)
        ...
        for step, batch in enumerate(epoch_iterator):
            with autocast():
                ...
                inputs = {"input_ids": batch[0], "attention_mask": batch[1], "labels": batch[3],
                          "type_index_a": batch[6], "type_index_b": batch[7], "logits": batch[5]}
...
def main(args):
    ...
    args.output_dir = 'model_distill_results/' + args.output_dir + '_' + args.model_type + '_' + 'multi_task' + '_seed_' + str(
        args.seed)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--lamb", default=0.9, type=float, help="lambda for distill",
    )
    parser.add_argument(
        "--temperature", default=1., type=float, help="temperature for distill",
    )
