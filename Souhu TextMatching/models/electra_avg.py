...
class ElectraTextClassificationWithAveragePooling(nn.Module):
    def __init__(self, args, model_path, num_labels=2, dropout=0.2):
        super(ElectraTextClassificationWithAveragePooling, self).__init__()
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, labels):
        ...
        hidden_states = discriminator_hidden_states[0]
        average = self.dropout(torch.mean(hidden_states, dim=1))
        average = self.fc(average)
        average = self.dropout(average)
        logits = self.out_proj(average)
