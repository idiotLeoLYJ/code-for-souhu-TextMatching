...
DATA_DIR = 'merged_data/all_train_multi_task_data.json'
...
def inference(args):
    ...
    with torch.no_grad():
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            preds.extend(np.array(logits.detach().cpu()))
    
    for item, pred in zip(test_data, preds):
        temp = {}
        temp['data'] = item
        temp['logits'] = (str(pred[0]), str(pred[1]))
        results.append((temp))
    
    args.output_path = args.output_dir + args.trained_model_path.split('/')[1] + '_infer_logits_' + str(f1)[:6] + '.json'
...
if __name__ == '__main__':
    ...
    parser.add_argument("--output_dir", default='logits_data/', type=str, help="The output directory")
